...
- **ローカルで「Local: http://localhost:3000」も見れない** → ターミナルで `npm run dev` を実行し、表示が出るまで待ってからブラウザで http://localhost:3000 を開く。
- **本番アプリに何の変化もない** → 変更を GitHub に push していないと本番には出ません。`git add .` → `git commit -m "..."` → `git push` のあと、1分ほど待ってから本番 URL を **Ctrl+Shift+R** でスーパーリロードする。

詳しい手順は **[docs/変更を反映させる手順.md](./docs/変更を反映させる手順.md)** に書いてあります。
