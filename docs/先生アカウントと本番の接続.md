...
7. `supabase/migrations/20250222600000_accompanist_availabilities_anon_set.sql`  
   → 伴奏者が「可能」枠を保存できる RPC（先生・生徒に反映するため）。
...
## 先生の設定が生徒・伴奏者に反映されない（レッスンOK日にしても見えない）

**原因になりやすいこと**

1. **Supabase の anon 用ポリシーが未適用**  
   生徒・伴奏者は「名前を選択して入る」だけで Supabase にはログインしていません（anon のまま）。  
   そのため、**anon が day_settings などを読めるポリシー**がないと、先生が設定した「レッスン実施日」などを取得できません。  
   - **対応**: マイグレーション **20250222500000_teacher_only_register_anon_read.sql** を本番プロジェクトで実行しているか確認する。  
   - このファイルで `day_settings_select_anon` などが作成され、anon でも日設定・レッスン・週間マスター・可能枠を読めるようになります。

**anon 用ポリシーが入っているか確認する方法**

1. **https://supabase.com/dashboard** を開き、本番で使っているプロジェクトを選択する。
2. 左メニューで **Database** → **Tables** を開き、**day_settings** をクリックする。  
   テーブル詳細の **Policies**（または RLS のポリシー一覧）タブを開く。  
   （UI によっては **Authentication** → **Policies** でテーブル別にポリシーを確認できる）
3. ポリシー一覧に **day_settings_select_anon** があるか確認する。  
   - **ある** → anon 用の読取ポリシーは入っている。反映されない場合は「最新に更新」や 2 分待ち、別原因を確認する。  
   - **ない** → マイグレーション未実行。下記「SQL で一括確認」のあと、**20250222500000_teacher_only_register_anon_read.sql** の内容を SQL Editor で実行する。

**SQL で一括確認する方法**

Supabase の **SQL Editor** で次を実行する。

```sql
SELECT tablename, policyname, roles
FROM pg_policies
WHERE schemaname = 'public'
  AND policyname IN ('day_settings_select_anon', 'lessons_select_anon', 'weekly_masters_select_anon', 'accompanist_availabilities_select_anon');
```

- **4 行返る**（4 つのポリシーがそれぞれ 1 行ずつ）→ anon 用ポリシーは適用済み。  
- **0 行や 4 行に満たない** → 不足しているポリシーがある。**20250222500000_teacher_only_register_anon_read.sql** を SQL Editor に貼って実行する。

2. **反映のタイミング**  
   - 生徒・伴奏者側は **2分ごとに自動で** サーバーから最新の日設定・レッスンを再取得します。  
   - すぐ反映させたい場合は、サイドバー下部の **「最新に更新」** を押してください。
