...
import { getLessonSlotList, today } from '@/lib/schedule'
...
export default function WeeklyMasterPage() {
  ...
  const defaultYearRange = getAcademicYearRange()
  const [applyFromDate, setApplyFromDate] = useState(() => today())
  const [applyToDate, setApplyToDate] = useState(() => defaultYearRange.end)
  ...
  /** 週間マスターを指定期間のカレンダー枠に反映（期間外・過去データはそのまま残す） */
  const handleApplyToCalendar = () => {
    if (applyFromDate > applyToDate) {
      setFeedback({ ok: false, text: '反映期間の開始日は終了日以前にしてください。' })
      return
    }
    const next = buildMastersFromLocalMap(localMap)
    dispatch({ type: 'REPLACE_WEEKLY_MASTERS', payload: next })
    dispatch({
      ...
      payload: { effectiveFromDate: applyFromDate, effectiveToDate: applyToDate },
    })
    setSaveNonce((n) => n + 1)
    setFeedback({
      ok: true,
      text: `カレンダーに反映しました（${applyFromDate} 〜 ${applyToDate}）。この期間外のレッスンは変更しません。保存は数秒以内にサーバーへ同期されます。`,
    })
  }
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          onClick={() => {
            const r = getAcademicYearRange()
            setApplyFromDate(r.start)
            setApplyToDate(r.end)
          }}
        >
          今年度（{formatAcademicYearLabel(defaultYearRange.start, defaultYearRange.end)}）をセット
        </button>
        {feedback && (
          <span
