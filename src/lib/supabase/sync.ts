...
export interface ActivityLogEntry {
  ...
  details: { studentName?: string; accompanistName?: string; slotCount?: number } | null
}
...
export interface InsertActivityLogParams {
  ...
  details?: { studentName?: string; accompanistName?: string; slotCount?: number }
}

export async function insertActivityLog(
  supabase: NonNullable<ReturnType<typeof import('./client').createSupabaseClient>> | null,
  ...
  try {
    await supabase.from('activity_logs').insert({
      ...
      lesson_date: params.lessonDate ?? null,
      lesson_start_time: params.lessonStartTime ?? null,
...
export async function fetchActivityLogs(
  ...
  const { data } = await supabase
    .from('activity_logs')
    .select('id, created_at, actor_id, actor_name, action, lesson_id, lesson_date, lesson_start_time, details')
  ...
  return data.map((r: {
    ...
    lessonStartTime: r.lesson_start_time,
    details: r.details as ActivityLogEntry['details'],
  }))
}

/** 操作ログを1件削除。RLS により先生のみ成功する */
